...
def get_wid(pagination_str,timeStamp,oid):
...
def read_url(pagination_str = "{\"offset\":\"\"}",oid = "747023094") :
    '''返回下一个page_id'''
    #参数部分
    timeStamp = int(time.time() * 1000)
    w_rid = get_wid(pagination_str,timeStamp,oid)
...
if __name__ == "__main__":
    ...
    oid = "451979316"
    next_pagination_str = read_url(pagination_str,oid)
    time.sleep(random.randint(0,3))
    page_count = 18
    for index in range(page_count):
        next_pagination_str = read_url(next_pagination_str,oid = oid)
        time.sleep(random.randint(0,3))
